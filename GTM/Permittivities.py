...
def eps_KRS5(f):
    ...
    if (f<fn_KRS5.min()) or (f>fn_KRS5.max()):
...
def eps_GaNx(f): # GaN ordinary epsilon
...
def eps_SiN(f):
    ### https://arxiv.org/pdf/1209.2987.pdf
    omega = f*(2*np.pi)
...
def eps_Ag(f):
    ## all from Yang et al., Phys. Rev. B 91, 235137 (2015)
    # doi.org/10.1103/PhysRevB.91.235137
...
def eps_BaF2(f):
    ...
    # https://apps.dtic.mil/docs/citations/ADA192210
    # downloaded from RefractiveIndex.info (2019)
...
def eps_SiO2(f):
    ...
    # and Malitson, J. Opt. Soc. Am. 55, 1205 (1965)
    # downloaded from RefractiveIndex.info (2019)
